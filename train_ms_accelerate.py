    ...
    for batch_idx, (
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                # 构建tqdm描述
                losses_name = ['loss_disc', 'loss_gen', 'loss_fm', 'loss_mel', 'loss_dur', 'loss_kl']
                losses_value = [x.item() for x in losses]
                tqdm_str = f'Steps: {{global_step}} | LR: {{lr:.2e}} | '
                tqdm_str += ' | '.join([f'{{name}}: {{val:.4f}}' for name, val in zip(losses_name, losses_value)])
                
                if net_dur_disc is not None:
                    tqdm_str += f' | loss_dur_disc: {{loss_dur_disc_all.item():.4f}}'
                if net_wd is not None:
                    tqdm_str += f' | loss_slm: {{loss_slm.item():.4f}}'
                
                pbar.set_description(tqdm_str)
                epoch_loss.append(losses_value + [loss_dur_disc_all.item() if net_dur_disc else 0, loss_slm.item() if net_wd else 0])
